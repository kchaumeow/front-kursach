...
export function Home() {
  ...
  if (!movies.length)
    return (
      <>
        <Text textAlign="center" p={2} fontSize="large">
          База фильмов пуста
        </Text>
        <Heading mt="70px" textAlign="center" mb={4}>
          Спарсить фильм по id кинопоиска с его страницы
        </Heading>
  ...
  return (
    <Box display="flex" flexDirection="column" alignItems="center">
      {movies.map((movie) => (
        <div key={movie.id}>
          <MovieCard movie={movie} reload={reloadMovies} />
        </div>
      ))}
      <Link to="/movies/create">
        <Button colorScheme="teal">Добавить фильм</Button>
      </Link>
      <Heading mt="70px" mb={4}>
        Спарсить фильм по id кинопоиска с его страницы
      </Heading>
      <Parsing reload={reloadMovies} />
    </Box>
