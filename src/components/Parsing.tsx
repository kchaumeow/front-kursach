...
const ids = ["326", "342", "361", "370", "430", "435", "448", "49684", "258687", "535341"]
			...
			<Box mb={300}>
				<form onSubmit={(e)=> {
					...
					<Box display="flex" flexDirection="column" gap={5} alignItems="center">
						<Select w="400px" value={id} placeholder='Выбрать id фильма с КиноПоиска' required onChange={(e) => {
							setId(e.target.value)
						}}>
							{ids.map((el) => <option key={el} value={el}>{el}</option>)}
						</Select>
						<Button type="submit" disabled={isLoading} colorScheme='blue'>Спарсить фильм</Button>
					</Box>
				</form>
			</Box>
    )
